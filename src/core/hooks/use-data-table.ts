...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
	...
	const initialColumnFilters: ColumnFiltersState = React.useMemo(() => {
		...
		return Object.entries(filterValues).reduce<ColumnFiltersState>(
			(filters, [key, value]) => {
				if (value !== null) {
					// For array values, keep as is. For string values, use as a single value
	...
	const table = useReactTable({
		...
		filterFns: tableProps.filterFns,
		...
		// set true when using server-side pagination, sorting or filtering
		manualPagination,
		manualSorting,
		manualFiltering
	})
